...
DELIMITER //
CREATE PROCEDURE add_person(p_cik VARCHAR(20), p_name VARCHAR(20), p_LP VARCHAR(20))
BEGIN
insert into person (cik, name, LP)
values (p_cik, p_name, p_LP)
on duplicate key update LP = p_LP;
END//
DELIMITER ;
...
on duplicate key update date = p_date, url = p_url;
END//
DELIMITER ;
